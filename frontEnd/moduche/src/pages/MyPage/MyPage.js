// src/pages/main/Main.jsx (파일 위치는 네 프로젝트 구조에 맞게)
...
const Main = () => {
  ...
  // ✅ 동아리 테이블 셀 클릭 → 동아리 게시판으로 이동 + 검색
  const handleClickCommunity = (id, column, row) => {
    const keyword = row[column];
    if (!keyword) return;

    navigate(`/community/board?keyword=${encodeURIComponent(keyword)}`);
  };

  // ✅ 시설 테이블 셀 클릭 → 강좌 게시판으로 이동 + 검색
  const handleClickFacility = (id, column, row) => {
    const keyword = row[column];
    if (!keyword) return;

    navigate(`/course/board?keyword=${encodeURIComponent(keyword)}`);
  };
            ...
            <Layout space={2}>
              <Grid size={12}>
                ...
                <CustomTable
                  ...
                  clickEvent={handleClickFacility}
                />
              </Grid>
