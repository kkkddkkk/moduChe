...
const CustomTable = ({
  ...
  const format = (column, data, formatting) => {
    ...
    return formatValue ? formatValue[column](data) : data;
  };
          ...
          return (
            ...
            >
              {columns.map((column, columnIdx) => {
                return (
                  <TableCell
                    key={`row_${dataIdx}__column${columnIdx}`}
                    sx={{
                      textAlign: 'center',
                      cursor: hover.includes(column) ? 'pointer' : '',
                      color: hover.includes(column) ? hoverColor || mainColor : '',
                      padding: padding,
                    }}
                    onClick={
                      hover.includes(column)
                        ? () => clickEvent(data[id], column, data[column])
                        : () => {}
                    }
                  >
